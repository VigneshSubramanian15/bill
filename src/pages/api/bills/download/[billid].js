...
export default async function handler(req, res) {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ error: "Missing bill id in query parameters" });
    }
    ...
    const { name, address, phoneNumber } = await Company.findById(
      billData.companyId,
    ...
    const pdfArrayBuffer = generateInvoicePdf(
      companyInfo,
      billData,
      false,
      true,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Invoice_${billData.billNumber}.pdf`,
    );
